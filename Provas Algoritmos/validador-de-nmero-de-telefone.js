...
//Seu trabalho é validar ou rejeitar o número de telefone dos EUA com base em qualquer combinação dos formatos fornecidos acima. 
//O código de área é obrigatório. Se o código do país é fornecido, você deve confirmar que o código do país é 1.
// Retorne true se a seqüência de caracteres for um número válido de telefone dos EUA; caso contrário, retorne false.

//------------------------------------------------------------------------------------------------------------------

function telephoneCheck(str) {
  let regex = /^(1\s?)?(\d{3,3}||\(\d{3,3}\))(-||\s||^\s)\d{3,3}(-||\s||^\s)\d{4,4}$/
  let validated = false;

  let length = str.split(/[-\s\(\)]/).join("").split("").length;
  if(regex.test(str) && (length == 10 || length == 11)){
    validated = true
  }
  return validated;
}
